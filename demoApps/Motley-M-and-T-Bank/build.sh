...
cd src/main/webapp/
zip -r ../../../yaml/artifacts/mandtbank.war ./*
...
xl apply --xl-deploy-url=$XLD_URL --xl-deploy-username=$XLD_USER --xl-deploy-password=$XLD_PASSWD --file motley-m-and-t-bank-app.yaml --values version=${VERSION}
if [ ! -x "$path_to_executable" ]
then
  rm xl
fi
cd ..
rm yaml/artifacts/mandtbank.war
